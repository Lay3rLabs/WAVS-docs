...
WAVS (Web Assembly Verifiable Services) is the event-driven framework for distributed architectures that helps teams ship decentralized, verifiable applications faster. Developers define events (onchain, offchain, or internal), trigger workflows that execute components via the WAVS Engine, and submit verifiable outputs onchain.

[WAVS](https://github.com/Lay3rLabs/WAVS) is a [WASI](./how-it-works#wasm-and-wasi) runtime for building verifiable services. With WAVS, you can create, manage, and operate high-performance verifiable apps using the languages you already know and love (Rust, Golang, JavaScript/TypeScript, C/C+ and Python). By providing a base layer of verifiable app infrastructure, WAVS lets you focus on implementing the core logic of your service. WAVS compiles that logic to [WASM](./how-it-works#wasm-and-wasi), and lets you deploy it as lightweight service components.
...
> In simple terms, WAVS help you ship verifiable applications faster.
...
This example will cover a basic verifibale service with three parts: a trigger, a service component, and submission logic.
...
The WAVS platform is secured via Ethereum restaking on EigenLayer, which provides a base security layer for applications built using WAVS. Restaking refers to the utilization of staked Ethereum to secure verifiable services by imposing additional slashing terms on the staked Ethereum for operator misbehavior. In this way, the cryptoeconomic security of Ethereum is extended to WAVS apps.

## Full-stack decentralization

WAVS enables full-stack decentralization by unifying off-chain computation with on-chain verifiability. The chain layer connects to Ethereum and other chains, while the security layer extends cryptoeconomic security via EigenLayer restaking. At the app layer, lightweight WAVS powers WASM-based services to process off-chain computations triggered by on-chain events. Operators validate, sign, and commit the results back on-chain, ensuring verifiability and trust-minimized execution. This architecture makes WAVS a scalable, decentralized framework for full-stack applications.
