...
## Deploy PoA Service Manager

This deploys the [PoA service manager contract](https://github.com/Lay3rLabs/poa-middleware), which controls the operator set for your service. An owner account (the deployer by default) manages which operators are registered and their relative voting weight.

```bash docci-delay-after=2
COMMAND=deploy task operator:deploy
```
...
# Deploy the service to WAVS so it now watches and submits.
WAVS_ENDPOINT=http://127.0.0.1:8000 SERVICE_URL=${IPFS_URI} IPFS_GATEWAY=${IPFS_GATEWAY} task deploy:service
```
...
With PoA, the deployer (owner) registers operators and assigns a weight. No ETH stake is required.

```bash
SERVICE_INDEX=0 source ./script/avs-signing-key.sh

# Local: read the PoA service manager address from the deploy output
export WAVS_SERVICE_MANAGER_ADDRESS=$(jq -r .addresses.POAStakeRegistry ./.nodes/poa_deploy.json)
# TESTNET: set WAVS_SERVICE_MANAGER_ADDRESS

# Derive the operator's EVM address from their private key
export OPERATOR_ADDRESS=$(cast wallet address --private-key "$OPERATOR_PRIVATE_KEY")

# Register the operator (owner-managed — the deployer account signs this tx)
OPERATOR_ADDRESS=${OPERATOR_ADDRESS} task operator:register

# Set the operator's signing key (the operator signs this tx with their own key)
OPERATOR_PRIVATE_KEY=${OPERATOR_PRIVATE_KEY} AVS_SIGNING_ADDRESS=${AVS_SIGNING_ADDRESS} task operator:update-signing-key
```
...
export SERVICE_TRIGGER_ADDR=`task get-trigger-from-deploy`
...
TRIGGER_ID=1 RPC_URL=${RPC_URL} task show-result
```
