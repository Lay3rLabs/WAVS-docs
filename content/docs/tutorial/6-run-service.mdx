...
This tutorial uses the [wavs-foundry-template](https://github.com/Lay3rLabs/wavs-foundry-template) and its Makefile-based workflow. Real-world WAVS projects may use different tooling and project structures.
...
```bash docci-delay-after=2
COMMAND=deploy make poa-middleware
```
...
COMMAND="owner_operation registerOperator ${OPERATOR_ADDRESS} 100" make poa-middleware

# Set the operator's signing key (the operator signs this tx with their own key)
COMMAND="update_signing_key ${OPERATOR_PRIVATE_KEY} ${AVS_SIGNING_ADDRESS}" make poa-middleware
```
