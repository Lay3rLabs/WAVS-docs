...
<Callout title="Linux essentials" type="info">
...
<Tabs groupId="language" items={['MacOS', 'Linux', 'Windows WSL']} persist>
  ...
  Install [Docker Desktop with WSL](https://docs.docker.com/desktop/wsl/#turn-on-docker-desktop-wsl-2), then run:
...
<Callout title="Avoid sudo for Docker" type="info">
...
Visit the [Task Documentation](https://taskfile.dev/) for more info.
...
Visit the [Compose Documentation](https://docs.docker.com/compose/) for more info.
...
Install the latest version of [pnpm](https://pnpm.io/installation). pnpm is used by `task setup` to install JavaScript dependencies.
