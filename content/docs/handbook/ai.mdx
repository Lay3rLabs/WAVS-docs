...
6. The agent will test its component for errors by running validation tests using `task validate-component COMPONENT=your-component`.

7. The agent may need to make changes after running the Validation tests. After making changes, the agent will build the component using `WASI_BUILD_DIR=components/my-component task -y build:wasi`.
...
WASI_BUILD_DIR=components/openai-response task -y build:wasi

# Once built, test it with:
INPUT_DATA="Only respond with yes or no: Is AI beneficial to the world?" COMPONENT_FILENAME=openai_response.wasm task wasi:exec
```

The agent may try to run the `task wasi:exec` command themselves. You should prompt the agent to give you the command instead, as it can't run the command without permissions.
...
- The agent is supposed to provide you with the `task wasi:exec` command. Sometimes it will try to run this itself and it will fail. Instead, ask it to give you the command.
- When copying and pasting the full `task wasi:exec` command, be careful with line breaks. You may need to reformat long lines to avoid breaking the command.
