...
| **Entry point** | `fn run(action: TriggerAction) -> Result<Vec<WasmResponse>, String>` | Three callbacks (see below) |
...
pub struct EvmSubmitAction {
    pub chain: String,           // Chain name as defined in wavs.toml
...
Each component (operator or aggregator) gets its own isolated KV bucket namespace. Data written by the aggregator component is not visible to operator components and vice versa.

</Callout>

## Building the aggregator component

Aggregator components are built the same way as operator components — compiled to WASM and uploaded to the registry. In the template, a separate `Makefile` target handles this:

```bash
make build-aggregator  # Compile aggregator component to WASM
make upload-aggregator # Upload to local registry
```
