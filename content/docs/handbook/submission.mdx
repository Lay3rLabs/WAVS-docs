...
<Callout title="Aggregator component required" type="warn">

As of WAVS v1.0.0, services that submit on-chain must include a custom aggregator component. The aggregator component controls submission timing and target addresses. See the [Aggregator component guide](./components/aggregator) for how to build one.
...
WAVS currently uses ECDSA signatures for aggregation.
